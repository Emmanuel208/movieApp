...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var newArrayList : ArrayList<Movie1>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        newArrayList = arrayListOf<Movie1>()
        getUserdata()
    ...
    private fun getUserdata() {
        for (i in image.indices){
            val news = Movie1(image[i], title[i], description[i], genres[i], runningTime[i], movieSeatsRemaining[i])
